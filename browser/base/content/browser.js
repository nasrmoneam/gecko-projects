...
var gMenuButtonUpdateBadge = {
  ...
  get downloadPromptAttempts() {
    return Services.prefs.getIntPref("app.update.download.promptAttempts", 0);
  },

  set downloadPromptAttempts(value) {
    Services.prefs.setIntPref("app.update.download.promptAttempts", value);
  },

  get downloadPromptMaxAttempts() {
    let pref = Services.prefs.getIntPref("app.update.download.promptMaxAttempts", 2);

    let maxVal = 10;
    let minVal = 0;
    return Math.min(maxVal, Math.max(pref, minVal));
  },

  get elevationPromptAttempts() {
    return Services.prefs.getIntPref("app.update.elevation.promptAttempts", 0);
  },

  set elevationPromptAttempts(value) {
    Services.prefs.setIntPref("app.update.elevation.promptAttempts", value);
  },

  get elevationPromptMaxAttempts() {
    let pref = Services.prefs.getIntPref("app.update.elevation.promptMaxAttempts", 2);
  ...
  showUpdateNotification(type, dismissed, callback) {
    let action = {
  ...
  handleUpdateError(update, status) {
    switch (status) {
      case "all-downloads-failed":
        this.downloadPromptAttempts++;
        this.clearCallbacks();

        if (this.downloadPromptAttempts > this.downloadPromptMaxAttempts) {
      ...
      case "elevation-canceled":
        this.elevationPromptAttempts++;
        this.clearCallbacks();
        if (this.elevationPromptAttempts > this.elevationPromptMaxAttempts) {
          this.showManualUpdateNotification(update, false);
        } else {
