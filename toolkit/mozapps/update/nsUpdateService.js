...
class ChannelDownloader extends CommonDownloader {
  ...
  downloadUpdate(update) {
    ...
    this._bkgFileSaver.setTarget(patchFile, false);
  ...
  onStatus(request, context, status, statusText) {
    LOG("ChannelDownloader:onStatus - status: " + status + ", statusText: " +
        statusText);
  ...
  onProgress(request, context, progress, maxProgress) {
    ...
    if (maxProgress != this._patch.size) {
      LOG("ChannelDownloader:onProgress - maxProgress: " + maxProgress +
  ...
  onDataAvailable(request, context, stream, offset, count) {
    // this._bkgFileSaver may not exist anymore if we've been cancelled.
    if (this._bkgFileSaver) {
    ...
    if (Components.isSuccessCode(status)) {
      this._bkgFileSaver.observer = {
        ...
        onSaveComplete: (aSaver, aStatus) => {
          this._bkgFileSaver.observer = null;
          this.finishDownload(request, context, aStatus);
        }
      };
      this._bkgFileSaver.finish(status);
    } else {
      this.finishDownload(request, context, status);
    }
  }

  finishDownload(request, context, status) {
