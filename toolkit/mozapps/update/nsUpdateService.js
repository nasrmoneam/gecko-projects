...
function UpdateManager() {
  ...
  if (tmpFile.exists()) {
    ...
    activeUpdates = this._loadXMLFileIntoArray(FILE_ACTIVE_UPDATE_XML + ".tmp");
    if (activeUpdates.length > 0) {
      ...
      try {
        ...
        LOG("UpdateManager: error replacing active-update.xml file with " +
            "active-update.xml.tmp file. Exception: " + e);
      }
    } else {
      try {
        tmpFile.remove(false);
      } catch (e) {
        LOG("UpdateManager: error removing active-update.xml.tmp file. " +
            "Exception: " + e);
      }
    }
  }
  ...
  if (activeUpdates.length == 0) {
    // An active update wasn't loaded from the active-update.xml.tmp file so
    // load the active-update.xml file to try to load the active update.
    activeUpdates = this._loadXMLFileIntoArray(FILE_ACTIVE_UPDATE_XML);
  }
  if (activeUpdates.length > 0) {
...
UpdateManager.prototype = {
  ...
  _writeUpdatesToXMLFile: async function UM__writeUpdatesToXMLFile(updates, fileName) {
    ...
    try {
      ...
      LOG("UpdateManager:_writeUpdatesToXMLFile - error: " + e);
    }
  },
